...
import { Hud, makeHitButton, makeSoundToggle, makeText } from "./UI.js";
...
export class GameplayScene extends BaseScene {
  ...
  update(dt) {
    ...
    if (!this.crashed && this.spawnTimer <= 0) {
      this.spawnOilWave();
      this.spawnTimer =
        clamp(1.12 - this.elapsed * 0.022, 0.34, 1.12) +
        rand(0, 0.22);
    }
    ...
    if (this.crashed) {
      ...
      if (this.crashTimer > 0.58) {
        this.game.endRun(this.score);
      }
    }
  }
  ...
  spawnOilWave() {
    ...
    for (let i = 0; i < count; i += 1) {
      const oil = Oil.randomSpawn(
        this.game.oilTextures,
        this.elapsed + i * 2
      );

      if (count === 2) {
        oil.container.x += i === 0 ? -260 : 260;
      }
